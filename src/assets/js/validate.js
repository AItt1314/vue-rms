export default {
    ...
    money: (rule, value, callback) => {
        ...
        const pass = /(^([0-9]{1,18})?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/.test(value)
        if (isNaN(value)) {
            return callback(new Error('必须为数字'));
        }
        if (String(value).split('.').length > 0) {
            if (String(value).split('.')[0].length > 18) {
                return callback(new Error('整数部分不能大于18位'));
            }
            if (value !== 0) {
                if (!pass || !value) {
                    return callback(new Error('钱数精确两位小数'));
                }
            }
        }
        callback();
